...
const commentBlockVisability = (isVisable) => {
  document.getElementsByTagName("html")[0].classList.remove("cursor");
  const div = document.getElementById("comment-block");
...
const saved = window.localStorage.getItem("template");

saved ? (commentContainer.innerHTML = saved) : "";

const p = Array.prototype.slice.call(
  document.getElementsByClassName("page")
  );
...
function addComment(ev) {
  ...
  if (commentText) {
    const comment = document.getElementsByClassName("commentText");
    textBox.innerHTML = commentText;

    function clickComment() {
      selection.innerText = textBox.innerText;
    }
    wrapDiv.append(textBox, deleteButton);
    commentContainer.appendChild(wrapDiv);

    wrapDiv.addEventListener("click", clickComment, false);
  }
}

const comments = document.getElementsByClassName("wrapper");
let arr = [];
if (comments.length) {
  arr.push(comments);
  arr.forEach((e) => {
    for (let i = 0; i < e.length; i++) {
      e[i].addEventListener("click", () => {
        selection.innerText = e[i].childNodes[0].innerText;
        // console.log(e[0].childNodes[0]);
      });
    }
  });
}
